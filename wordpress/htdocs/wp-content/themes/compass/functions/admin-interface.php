...
function compass_optionsframework_add_admin() {
    global $query_string;
    $themename = compass_get_option('of_themename');
    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework') {
        if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
            $options = compass_get_option('of_template');
            compass_reset_options($options, 'optionsframework');
            header("Location: admin.php?page=optionsframework&reset=true");
            die;
        }
    }
    $of_page = add_theme_page($themename, 'Theme Options', 'edit_theme_options', 'optionsframework', 'compass_optionsframework_options_page', 'div');
    // Add framework functionaily to the head individually
    add_action("admin_print_scripts-$of_page", 'compass_load_only');
}
add_action('admin_menu', 'compass_optionsframework_add_admin');
/* ----------------------------------------------------------------------------------- */
/* Options Framework Reset Function - of_reset_options */
/* ----------------------------------------------------------------------------------- */
function compass_reset_options($options, $page = '') {
    delete_option('compass_options');
}
/* ----------------------------------------------------------------------------------- */
/* Build the Options Page - optionsframework_options_page */
/* ----------------------------------------------------------------------------------- */
function compass_optionsframework_options_page() {
    $options_array = compass_theme_options();
	$options = $options_array['of_template'];
    $themename = $options_array['theme_name'];
    ?>
    <div class="trail-notify">
        <h1>Get Compass PRO!</h1>
        <p style="font-size:15px; line-height: 20px;">You are using the free version of Compass theme. Upgrade to pro for extra features like home page slider, featured section, testimonial, boxed blog layout, fullWidth, contact page templates and many other, multiple color options, premium theme support and much more.</p>
        <a href="http://www.inkthemes.com/wp-themes/featured-content-slider-wordpress-theme/" target="blank">Upgrade to Compass PRO here >></a>
    </div>
    ...
    $return = compass_optionsframework_machine($options);
    ?>
            <div id="main">
                <div id="of-nav">
                    <ul>
        ...
        foreach ($options as $option) {
            if ($option['type'] == 'color' OR $option['type'] == 'typography' OR $option['type'] == 'border') {
                if ($option['type'] == 'typography' OR $option['type'] == 'border') {
                    $option_id = $option['id'];
                    $temp_color = compass_get_option($option_id);
                    $option_id = $option['id'] . '_color';
                    $color = $temp_color['color'];
                } else {
                    $option_id = $option['id'];
                    $color = compass_get_option($option_id);
                }
                ?>
...
/* ----------------------------------------------------------------------------------- */
/* Ajax Save Action - compass_ajax_callback */
/* ----------------------------------------------------------------------------------- */
add_action('wp_ajax_of_ajax_post_action', 'compass_ajax_callback');

function compass_ajax_callback() {
    ...
    if ($save_type == 'upload') {
        ...
        compass_update_option($clickedID, $uploaded_file['url']);
        ...
        //print_r($output);
        //Pull options
        $default_options = compass_theme_options();
        foreach ($default_options['of_template'] as $option_array) {
            $id = $option_array['id'];
            $old_value = compass_get_option($id);
            $new_value = '';
                ...
                if (is_array($type)) {
                    foreach ($type as $array) {
                        if ($array['type'] == 'text') {
                            ...
                            compass_update_option($id, stripslashes($new_value));
                        }
                    }
                } elseif ($new_value == '' && $type == 'checkbox') { // Checkbox Save
                    compass_update_option($id, 'false');
                } elseif ($new_value == 'true' && $type == 'checkbox') { // Checkbox Save
                    compass_update_option($id, 'true');
                    ...
                    foreach ($option_options as $options_id => $options_value) {
                        ...
                        if (!isset($output[$multicheck_id])) {
                            compass_update_option($multicheck_id, 'false');
                        } else {
                            compass_update_option($multicheck_id, 'true');
                        }
                    }
                    ...
                    compass_update_option($id, $typography_array);
                } elseif ($type == 'border') {
                    ...
                    compass_update_option($id, $border_array);
...
function compass_optionsframework_machine($options) {
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case 'text':
                $val = $value['std'];
                $std = compass_get_option($value['id']);
                if ($std != "") {
                    $val = $std;
                }
            ...
            case 'select':
                ...
                $select_value = compass_get_option($value['id']);
            ...
            case "upload":
                $value['std'] = '';
                if (isset($value['std'])) {
                    $output .= compass_optionsframework_uploader_function($value['id'], $value['std'], null);
                }
                break;
            case "upload_min":

                $output .= compass_optionsframework_uploader_function($value['id'], $value['std'], 'min');

                break;
            case "color":
                $val = $value['std'];
                $stored = compass_get_option($value['id']);
                if ($stored != "") {
                    $val = $stored;
                }
            ...
            case "typography":
                ...
                $default = $value['std'];
                $typography_stored = compass_get_option($value['id']);
            ...
            case "border":
                ...
                $default = $value['std'];
                $border_stored = compass_get_option($value['id']);
        ...
        if (is_array($value['type'])) {
            foreach ($value['type'] as $array) {
                ...
                $id = $array['id'];
                $std = $array['std'];
                $saved_std = compass_get_option($id);
                if ($saved_std != $std) {
                    $std = $saved_std;
                }
                $meta = $array['meta'];
...
/* ----------------------------------------------------------------------------------- */
/* OptionsFramework Uploader - compass_optionsframework_uploader_function */
/* ----------------------------------------------------------------------------------- */

function compass_optionsframework_uploader_function($id, $std, $mod) {
    ...
    $uploader = '';
    $upload = compass_get_option($id);

    if ($mod != 'min') {
        $val = $std;
        if (compass_get_option($id) != "") {
            $val = compass_get_option($id);
        }
